...
def save_report(results: List[Dict[str, Any]], output_path: str) -> None:
    ...
    2. Secondary: category scores in weight order (descending)
       - core_lrd_focus (0.40)
       - observational_techniques (0.20)
       - physical_properties (0.15)
       - scientific_impact (0.15)
    ...
    sorted_results = sorted(
        ...
        key=lambda p: (
            ...
            -get_category_score(p, "core_lrd_focus"),  # Secondary 1: weight 0.40
            -get_category_score(p, "observational_techniques"),  # Secondary 2: weight 0.20
            -get_category_score(p, "physical_properties"),  # Secondary 3: weight 0.15
            -get_category_score(p, "scientific_impact"),  # Secondary 4: weight 0.15
            -get_category_score(p, "methodological_rigor"),  # Secondary 5: weight 0.10
        ),
    )
...
def main():
    ...
    parser.add_argument(
        "--output",
        default=str(repo_root / "AIBot" / "results" / "ranking_report.json"),
        help="Output JSON report path (default: AIBot/results/ranking_report.json)",
    )
    parser.add_argument(
        "--criteria",
        default=str(repo_root / "AIBot" / "data" / "lrd_ranking_criteria.json"),
        help="Path to ranking criteria JSON (default: AIBot/data/lrd_ranking_criteria.json)",
    )
    parser.add_argument(
        "--limit",
        type=int,
